...
The toolset consists of 7 modular utilities:
...
- **Auto-Loading Cross-Tool Shared Calibration**:
  - Automatically fetches and loads `decoder/gear_calibration.json` via `GET /api/calibration` upon startup if present.
...
- **Auto-Loading Calibration on Startup**:
  - Automatically fetches and loads `decoder/gear_calibration.json` upon initialization if present on disk.
...
## 8. Unified Design System & Theme Styling

All web-based tools (`tuner.py`, `gear_lab.py`, `visualize.py`, `trim_log.py`, `cockpit_3d.py`, `streamer.py`) share a cohesive UI design language and color system engineered for automotive engineering clarity:
...
## 9. Requirements & Implementation Traceability

For the full architectural specification, functional requirement statements, and Implementation Traceability Matrix mapping each requirement (`REQ-DEC-*`, `REQ-VIS-*`, `REQ-TUN-*`, `REQ-LAB-*`, `REQ-TRIM-*`, `REQ-CPT-*`, `REQ-STR-*`, `REQ-SYS-*`) to exact code implementations and test procedures, refer to:

👉 **[REQUIREMENTS.md](REQUIREMENTS.md)**

---

## 10. Automated Test Suite (`decoder/tests/`)
...
python3 decoder/tests/test_streamer.py
```
