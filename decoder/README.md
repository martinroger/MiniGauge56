...
2. **Uncontainerized Dual-Screen Cockpit Cluster (`vx-binocle-espidf`)**:
   - **Left Screen (Tachometer)**: Needle-less glowing green perimeter arc ($0$–$8000$ RPM) mathematically aligned with tick graduations ($R=118$, sweep $135^\circ$ to $405^\circ$), central large numeric RPM readout (`rpm-glyph`), and lower readouts (`COOL`, `FUEL`).
   - **Right Screen (Speedometer)**: Needle-less glowing cyan perimeter arc ($0$–$240$ KPH) mathematically aligned with scale ticks, large central speed digits (`speed-glyph`), and lower readouts (Battery voltage, trip odometer, ground speed).
...
3. **Visual Effects & Smooth Camera Physics**:
   ...
   - **Interactive Mouse Wheel Zoom**: Smooth zoom in Chase view ($30\text{m}$ bumper cam to $550\text{m}$ scenic chase) and Top view ($120\text{m}$ close track to $4500\text{m}$ regional overview), alongside native OrbitControls in Free mode.
