...
(function () {
  ...
  const chkParseDids = document.getElementById('chk-parse-dids');
  ...
  const formAddDid = document.getElementById('form-add-did');
  ...
  function setupEventListeners() {
    ...
    chkParseDids.addEventListener('change', () => {
      if (currentLogFile) {
        fetchExchanges();
      }
    });
    ...
    formAddDid.addEventListener('submit', handleSaveNewDid);
  }
  ...
  function fetchExchanges() {
    ...
    const parseDids = chkParseDids.checked;
    ...
    fetch(`/api/exchanges?file=${encodeURIComponent(currentLogFile)}&parse_dids=${parseDids}`)
      .then((res) => {
  ...
  function matchesFilter(item) {
    ...
    if (activeFilter === 'ddli' && item.service_id !== '0x2C' && item.service_id !== '0x6C') return false;
  ...
  function buildSummaryText(item) {
    ...
    if (item.service_id === '0x2C') {
      ...
      return `DDLI ${details.local_id || ''} [${details.subfunction_name || ''}] ${entryCount} DIDs (${details.total_bytes || 0} bytes)`;
    }
    if (item.service_id === '0x6C') {
      return `DDLI ${details.local_id || ''} configured OK`;
    }
    ...
    if (item.service_id === '0x61') {
      if (details.signals && details.signals.length > 0) {
        return details.signals.map((s) => `${s.did_name}: ${s.scaled_value} ${s.unit}`).join(' | ');
      }
    ...
    if (!item.is_implemented) {
      return `Unknown Service (Payload: ${item.raw_hex.substring(2, 14)}...)`;
    }
    return '';
  }
  ...
  function renderDetailPane() {
    ...
    // 3. DDLI Configuration Block
    if (details.local_id && details.entries) {
      let entriesRows = details.entries.map((e) => `
        <tr>
          <td><strong style="color: var(--primary);">${escapeHtml(e.did_name)}</strong></td>
          <td><code>${e.did_hex}</code></td>
          <td>${e.memory_size} Byte(s)</td>
          <td>Pos ${e.position}</td>
          <td>${e.is_known ? '<span style="color: #10b981;">✓ Known in dids.json</span>' : '<span style="color: #f59e0b;">⚠️ Unknown DID</span>'}</td>
        </tr>
      `).join('');

      let unknownPrompts = (details.unknown_dids || []).map((uk) => `
        <div class="discovered-did-card">
          <div>
            <strong>Discovered New DID:</strong> <code>${uk.id}</code> (${uk.memory_size} bytes, position ${uk.position})
          </div>
          <button type="button" class="btn btn-primary btn-sm btn-prompt-add-did"
      ...
      contentHtml += `
        <div class="detail-section-card">
          <div class="detail-section-title">
            Dynamically Defined Local Identifier (DDLI ${details.local_id}) Configuration
          </div>
    ...
    // 4. Decoded Signals Table (from 0x61 or periodic responses, including all configured DIDs)
    if (details.signals && details.signals.length > 0) {
      let signalsRows = details.signals.map((sig) => `
        <tr>
          <td><strong>${escapeHtml(sig.did_name)}</strong> (<code>${sig.did_hex}</code>)</td>
    ...
    if (item.can_frames && item.can_frames.length > 0) {
      ...
      contentHtml += `
        <details class="can-frames-accordion">
          <summary>
            <span>Underlying CAN Frames (${item.can_frames.length})</span>
          </summary>
          <div class="can-frames-accordion-content">
            <table class="signals-table">
              <thead>
                <tr>
                  <th>Frame</th>
    ...
    // Bind prompt buttons for adding unknown DIDs
    document.querySelectorAll('.btn-prompt-add-did').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        const id = btn.getAttribute('data-id');
        const size = btn.getAttribute('data-size');
        const pos = btn.getAttribute('data-pos');
        openAddDidModal(id, size, pos);
      });
    });
  }
  ...
  function openAddDidModal(hexId, size, pos) {
    document.getElementById('did-id').value = hexId || '';
    document.getElementById('did-size').value = size || '2';
    document.getElementById('did-pos').value = pos || '1';
    document.getElementById('did-name').value = '';
    document.getElementById('did-unit').value = '';
    document.getElementById('did-mul').value = '1';
    document.getElementById('did-div').value = '1';
    document.getElementById('did-add').value = '0';

    modalAddDid.classList.add('open');
  }

  function closeModal() {
    modalAddDid.classList.remove('open');
  }

  function handleSaveNewDid(e) {
    e.preventDefault();

    const payload = {
      name: document.getElementById('did-name').value.trim(),
      id: document.getElementById('did-id').value.trim().toUpperCase(),
      memory_size: parseInt(document.getElementById('did-size').value, 10),
      position: parseInt(document.getElementById('did-pos').value, 10),
      unit: document.getElementById('did-unit').value.trim(),
      mul: parseInt(document.getElementById('did-mul').value, 10),
      div: parseInt(document.getElementById('did-div').value, 10) || 1,
      add: parseInt(document.getElementById('did-add').value, 10),
    };
    ...
    fetch('/api/add_did', {
      ...
      .then((resData) => {
        closeModal();
        showToast(resData.message || 'DID saved successfully', 'success');
        // Re-analyze active exchange log to reflect newly added DID
        fetchExchanges();
      })
      .catch((err) => {
        console.error('Error saving DID:', err);
        showToast(`Failed to save DID: ${err.message}`, 'error');
      });
  }
