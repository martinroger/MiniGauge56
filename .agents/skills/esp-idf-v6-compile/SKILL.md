---
name: esp-idf-v6-compile
description: Skill to configure, build, and qualitatively analyze ESP-IDF projects using framework versions v6.x and above. Includes support for eim tool manager, export.sh, MCP servers, breaking API changes audit, and standardized build diagnostic reporting.
---

# ESP-IDF v6.x+ Compilation & Qualitative Analysis Skill

This skill provides step-by-step guidance for configuring, compiling, and qualitatively analyzing ESP-IDF firmware projects targeting **ESP-IDF v6.x and above**.
...
### Mode B: Espressif Installation Manager (`eim`) (Recommended CLI)
For ESP-IDF v6.0+, `eim` is the standard virtual environment and toolchain manager:
...
1. **Set Target MCU Architecture** (if changing or initializing target):
   ...
   # Supported targets: esp32, esp32s2, esp32s3, esp32c2, esp32c3, esp32c6, esp32h2, esp32p4, esp32c5
   ```
...
- **v6.x Breaking Changes & Deprecations**: Detect usage of legacy driver APIs removed or restructured in v6.0 (e.g., legacy GPIO/I2C/SPI driver headers replaced by modern HAL/driver drivers).
...
```markdown
# ESP-IDF v6.x+ Qualitative Build Report
...
- **Total Warnings**: N
- **Deprecation / Breaking Change Warnings**: N
